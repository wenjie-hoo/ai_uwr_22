...
class Board:
    ...
    def copy(self):
        return Board(self.board, self.comandos, self.goals)
    def __repr__(self):
        return self.board
    def draw(self):
...
def elim_rows_cols(board, sequences=[('L', [2,3]), ('S', [3]), ('L', [0,1]), ('S', [2])]):
    board = board.copy()
    m = max(board.width-2, board.height-2)
    path = ''
    for l, seq in sequences:
        for i in range(m if l=='L' else m//2):
...
def best_elim(board):
    ...
    best = 10000000
    for sequences in sequencess:
        new_board, path = elim_rows_cols(board, sequences)
        if len(new_board.comandos) < best:
            best = len(new_board.comandos)
            best_board = new_board
            best_path = path
            best_sequences = sequences
    # print(best_sequences)
    return best_board, best_path
