            ...
            if new_pos in board and new_pos not in seen:
                if board[new_pos] == 'B':
                    ...
                    if new_pos+d in board and board[new_pos+d] != 'B':
                        walls = [p for p in around(new_pos+d) if board[p] == 'W']
...
def meta_search(board, goals, sokoban):
    queue = []
    ...
    while queue:
        ...
        for move in moves:
            ...
            if (new_board.boxes, new_min_position) not in seen:
                ...
                heappush(queue, (new_priority, new_history, new_moves, new_board, new_min_position))
...
with open("zad_input.txt", "r") as in_f:
    ...
    get_path(board, sokoban, meta_moves)
