...
def get_moves(w, r, b, turn):
    ...
    if turn == 1:
        for new_b in around(b):
            if on_board(new_b) and ch_dist(new_b, w)>1:
                if (new_b[0] == r[0] and not between(new_b, w, r, 1)):
                    continue
                if (new_b[1] == r[1] and not between(new_b, w, r, 0)):
                    continue
                moves.append((w, r, new_b, turn^1))
    if turn == 0:
        for new_w in around(w):
            if on_board(new_w) and ch_dist(new_w, b)>1 and new_w != r:
                moves.append((new_w, r, b, turn^1))
        for new_r in [(r[0], i) for i in range(8)] + [(i, r[1]) for i in range(8)]:
            if between(new_r, w, r): 
                continue
            if between(new_r, b, r):
                continue
    ...
    while queue:
        ...
        for move in get_moves(*last_move):
            if move not in seen:
                ...
                seen.add(move)
turn = 1 # Black to move, 0 for White to move
a = BFS("a1", "a6", "c3", turn)
# [draw(*line) for line in a]
